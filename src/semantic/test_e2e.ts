...
import { SemanticFormatter } from './formatters';
...
const testEndToEnd = async () => {
    ...
    if (!registry.getModel('vendas')) throw new Error("Model vendas not loaded");
    ...
    const { registry: globalRegistry } = await import('./registry');
    ...
    if (!sql.includes('WITH base_metrics AS')) throw new Error("SQL missing CTE");
